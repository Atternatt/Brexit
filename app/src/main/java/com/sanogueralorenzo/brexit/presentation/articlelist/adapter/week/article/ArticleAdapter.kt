...
class ArticleAdapter(val listener: OnArticleClickListener, private val items: ArrayList<ArticleItem>) : RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    inner class ArticleViewHolder(parent: ViewGroup) : RecyclerView.ViewHolder(
        ...
        fun bind(item: ArticleItem) = with(itemView) {
            ...
            articleTitleTextView.text = item.title ?: resources.getString(R.string.no_article_title)
            articleDateTextView.text = item.date?.dayMonthYearFormat() ?: resources.getString(R.string.no_article_date)
